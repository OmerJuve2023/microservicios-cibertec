...
| 2 | MS-Usuarios     | 8081   | `mvn spring-boot:run` (en ms-usuarios)     |
| 3 | MS-Clientes     | 8082   | `mvn spring-boot:run` (en ms-clientes)     |
| 4 | MS-Aplicaciones | 8083   | `mvn spring-boot:run` (en ms-aplicaciones) |
| 5 | MS-Menus        | 8084   | `mvn spring-boot:run` (en ms-menus)        |
| 6 | API Gateway     | 8080   | `mvn spring-boot:run` (en api-gateway)     |
...
> El token JWT **se agrega automáticamente** en cada petición gracias al `AuthInterceptor`. No es necesario agregarlo
> manualmente en cada servicio.
